...
describe('POST /api/template-rendering', () => {
  ...
  it('should return 401 if no authorization header is provided', async () => {
    ...
    try {
      await axios.post(`/api/template-rendering`, invoicePDFDocumentData);
    } catch (error) {
  ...
  it('generate an HTML document with the template and data passed in the request body', async () => {
    try {
      ...
      outputFile('invoice.html', res.data);
